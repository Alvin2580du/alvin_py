...
def get_action_features(step='train'):
    ...
    for root in [pos_root, neg_root]:
        ...
        for file in tqdm(os.listdir(root)):
            ...
            data_types = data['actionType'].values
            data_copy = data.copy()
            ...
            rows['17_atptp'] = np.ptp(time_counts)  # 时间极差
            rows['18_atvar'] = np.var(time_counts)  # 时间方差
            rows['19_xishu'] = np.mean(time_counts)/np.std(time_counts)  # 时间变异系数
            ...
            rows['29_atptp'] = np.ptp(data_types)  # 类型极差
            rows['30_atvar'] = np.var(data_types)  # 类型方差
            rows['31_xishu'] = np.mean(data_types)/np.std(data_types)  # 类型变异系数
            actions.append(rows)
