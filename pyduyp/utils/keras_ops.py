...
from keras.engine.training import losses, _standardize_input_data, _standardize_class_weights, _standardize_weights, _standardize_sample_weights
from keras.engine.training import _check_loss_and_target_compatibility, _slice_arrays
...
def _standardize_user_data(model, x, y,
    ...
    x = _standardize_input_data(x, model.input_names,
                               model.internal_input_shapes,
                               exception_prefix='model input')
    y = _standardize_input_data(y, model.output_names,
                               output_shapes,
    ...
    _check_loss_and_target_compatibility(y, model.loss_functions, model.internal_output_shapes)
...
def fit(model, x, y, batch_size=32, nb_epoch=10, verbose=1, callbacks=[],
    ...
    if validation_data:
        ...
        sample_weights, val_sample_weights = (
            _slice_arrays(sample_weights, 0, split_at), _slice_arrays(sample_weights, split_at))
        model._make_test_function()
        val_f = model.test_function
        if model.uses_learning_phase and type(K.learning_phase()) is not int:
            val_ins = val_x + val_y + val_sample_weights + [0.]
