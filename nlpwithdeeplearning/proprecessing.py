...
def makeNewTraingdatasets():
    labels = pd.read_csv("./results/predict.txt")
    unlabelcsv = pd.read_csv("./datasets/News_info_unlabel.csv", usecols=['msg'])
    unlabelcsvcopy = pd.DataFrame()
    unlabelcsvcopy['label'] = labels
    unlabelcsvcopy['msg'] = unlabelcsv['msg'].apply(jiebacut)
    unlabelcsvcopy.to_csv("./datasets/unlabelcsvcopy.csv", index=None, line_terminator='\n',
                          encoding="utf-8", sep=" ")
    print(unlabelcsvcopy.columns)


def concatcsv():
    print("concatcsvconcatcsvconcatcsvconcatcsvconcatcsvconcatcsv")
    unlabelcsv = pd.read_csv("./datasets/unlabelcsvcopy.csv", usecols=['msg', "label"])
    labelcsv = pd.read_csv("./datasets/News_info_train.csv", usecols=['msg', 'label'])
    datacopy = pd.DataFrame()
    datacopy['msgcut'] = labelcsv['msg'].apply(jiebacut)
    datacopy['label'] = labelcsv['label'].apply(addprefix)
    print("datacopydatacopydatacopydatacopydatacopydatacopy")
    df = pd.concat([datacopy, unlabelcsv], axis=0, join='outer', join_axes=None, ignore_index=True,
                   keys=None, levels=None, names=None, verify_integrity=False,
                   copy=True)

    df.to_csv("./datasets/Train_for_fasttext_Newdatasets.txt", index=None, line_terminator='\n',
              encoding="utf-8", header=None, sep=" ")
    print(df.columns)
    print('dfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdf')
...
if __name__ == "__main__":
    ...
    if method == 'makeNewTraingdatasets':
        makeNewTraingdatasets()
