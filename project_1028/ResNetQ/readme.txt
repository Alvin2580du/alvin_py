...
第一次使用resnet50预训练模型
python3 main.py --train_reocrd ./train_reocrd --resnet50_model_path ./models/resnet_v1_50.ckpt

当训练一段时间有logs目录下有checkpoint生成时，可以中途停止然后指定ckpt文件继续训练
python3 main.py --train_reocrd ./train_reocrd  --checkpoint_prefix ./checkpoint/model.ckpt-495


导出训练模型方便部署，会在指定的文件夹下生成 frozen_inference_graph.pb 可以供python及其它语言调用
python3 export_graph.py --checkpoint_prefix ./checkpoint/model.ckpt-495 --output_dir ./models
