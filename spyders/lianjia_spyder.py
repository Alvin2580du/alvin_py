        ...
        for i in range(len(resp)):
            ...
            try:
                ...
                resblocktype = housenames[0].findAll(
                    'span', attrs={'class': 'resblock-type'})[0].text
                salestatus = housenames[0].findAll(
                    'span', attrs={'class': 'sale-status'})[0].text
            except:
                continue
            try:
                resblocklocation = resp[i].findAll(
                    'div', attrs={'class': 'resblock-location'})
                addressinfolist = \
                    resblocklocation[0].text.replace("\n", "")
            ...
            try:
                resblockarea = resp[i].findAll(
                    'div', attrs={'class': 'resblock-area'})[0].\
                    text.replace("\n", "").replace("建面 ", "")
            except:
                continue
            try:
                ...
                priceinfo = resblockprice.findAll(
                    'span', attrs={'class': 'number'})[0].text
            except:
                continue
            try:
                secondprice = resp[i].findAll(
                    'div', attrs={'class': 'second'})[0].\
                    text.replace("总价", "").replace("万/套起", "")
            except:
                continue
